...
void move::printMove(int move, bool newLine) {
	std::cout << bitboard::squares[63 - GET_SOURCE(move)];

	int dest = GET_DEST(move);
	std::cout << bitboard::squares[63 - dest];

	int promote = GET_PROMOTE(move);
	if (promote)
		std::cout << bitboard::pieces[GET_PROMOTE(move) + bool(dest / 8) * 6];
